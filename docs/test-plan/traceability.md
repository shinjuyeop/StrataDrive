...
Status: M0 초안. **모든 runtime test는 NOT_RUN**이다. Result ID는 실행 후에만 부여한다.
아래는 coverage 후보이며 시험 완료의 증거가 아니다.

| Requirement | Architecture Component | Interface | Test Case | Result ID / 증거 | 검증 상태 |
| --- | --- | --- | --- | --- | --- |
...
변경 절차: Requirement 변경 이유와 version 기록 → component/interface 영향 확인
