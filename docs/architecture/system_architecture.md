...
Status: M0 **design draft**. 아래 구조는 Planned이며 동작 구현/성능 검증은 없다.
...
| ARCH-SAFETY | Target Central: Safety Supervisor | planner trajectory 검증, fallback/state 기반 제한, controller 출력 제한의 별도 guard |
...
| ARCH-DIAG | Target Central: Diagnostics Manager + Zone/vECU local monitors | fault event 집계, DTC, state transition; local 반응은 central 통신에 의존하지 않음 |
...
flowchart TB
  ...
  ACT --> BRIDGE[CAN feedback → Zone / target feedback bridge]
...
Controller가 CARLA API로 차량을 직접 움직이면 architecture 위반이다. Zone에서
보낸 command를 actual response로 간주하거나 adapter에서 command로 우회하는
...
writer는 planned integration에서 비활성화해야 한다.

vECU feedback은 CAN → Zone 수집/target bridge → Ethernet → Host adapter로
전달하는 계약 초안이다. bridge의 구체 구현/프로토콜은 TBD이며 Zone 내부
...
[안전 계약](../requirements/safety_requirements.md)에 따라 adapter 동작을 결정한다.
...
- 어떤 actuator 물리량/단위와 adapter mapping을 검증할 것인가?
- Zone feedback bridge, Host ingress transport와 clock 동기화는 어떻게 정할 것인가?
- 단일 Zone 시작 후 확장 여부, process/isolation 구성은 무엇인가?
