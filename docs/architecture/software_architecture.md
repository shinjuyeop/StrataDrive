...
| `central/planner` | Phase A route/waypoint, Phase C learned suggestion → candidate trajectory; classical fallback 유지 |
| `central/safety` | trajectory/state/health → validated trajectory, limits, selected fallback/system state |
...
| `central/diagnostics` | fault event → DTC lifecycle / system state decision |
...
| `zone/local_safety` | local timeout/fault → 제한 또는 local fail-safe; Central 의존 없는 판단 |
...
| `simulation/plant_adapter` | actual actuator feedback → plant-specific actuation mapping |
...
| `physical_ai/export`, `physical_ai/inference` | artifact provenance, export, TensorRT runtime; Central이 결과를 소비 |

Safety Supervisor가 planner와 controller 사이에서 trajectory를 검사한다.
Controller는 자체 출력 한계를 적용하고 Zone은 수신 command를 독립 검증한다.
...
| Candidate task | 예시 period (TBD) | Planned responsibility |
| --- | --- | --- |
...
| vECU | Command / dynamics candidates | Feedback / fault candidates |
| --- | --- | --- |
...
Dynamics parameter, sign/unit/frame, noise seed, initialization and calibration are
TBD. Accepted demand와 actual actuator state를 다른 interface로 유지한다. Simulated
current 등의 상태가 없으면 해당 fault는 검증됐다고 주장하지 않는다.

## Autonomous driving / Physical AI sequence

**Phase A — classical first (M8):** CARLA route/waypoint 기반 planner, Pure Pursuit
또는 Stanley lateral controller, Longitudinal PID. 반드시 Zone/vECU를 거친
